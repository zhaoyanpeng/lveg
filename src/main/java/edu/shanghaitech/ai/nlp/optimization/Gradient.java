...
public class Gradient extends Recorder implements Serializable {
	...
	public static class Grads {
		...
		public EnumMap<RuleUnit, List<List<Double>>> ggrads;
		public Grads(List<Double> wgrads, EnumMap<RuleUnit, List<List<Double>>> ggrads) {
			this.wgrads = wgrads;
			this.ggrads = ggrads;
		}
	}
	...
	protected EnumMap<RuleUnit, List<List<Double>>> ggrads, ggrads1, ggrads2, ggradst, ggradss;
	
	protected List<EnumMap<RuleUnit, List<List<List<Double>>>>> cachesWithT;
	protected List<EnumMap<RuleUnit, List<List<List<Double>>>>> cachesWithS;
	...
	private void initialize(GaussianMixture ruleW) {
		...
		this.updated = false;
		this.cachesWithT = ruleW.cachelike(2, 50);
		this.cachesWithS = ruleW.cachelike(10, 50);
	}
	...
	protected boolean apply(GrammarRule rule) {
		...
		GaussianMixture ruleW = rule.getWeight();
		ruleW.update(ggrads, wgrads, Optimizer.minexp);
		reset();
		return true;
	}
	...
	protected void reset() {
		wgrads.clear();
		for (Entry<RuleUnit, List<List<Double>>> unit : ggrads.entrySet()) {
			for (List<Double> grads : unit.getValue()) {
				grads.clear();
			}
		}
		cumulative = false;
	...
	protected void clearCaches() {
		for (EnumMap<RuleUnit, List<List<List<Double>>>> caches : cachesWithT) {
			for (Entry<RuleUnit, List<List<List<Double>>>> unit : caches.entrySet()) {
				for (List<List<Double>> cunit : unit.getValue()) {
					for (List<Double> gunit : cunit) {
						gunit.clear();
					}
				}
			}
		}
		for (EnumMap<RuleUnit, List<List<List<Double>>>> caches : cachesWithS) {
			for (Entry<RuleUnit, List<List<List<Double>>>> unit : caches.entrySet()) {
				for (List<List<Double>> cunit : unit.getValue()) {
					for (List<Double> gunit : cunit) {
						gunit.clear();
	...
	protected boolean eval(GrammarRule rule, Batch ioScoreWithT, Batch ioScoreWithS, List<Double> scoreSandT) {
		try {
			if (Optimizer.sampling) {
				throw new RuntimeException("Not supported parameter [sampling = true].");
			} else {
	...
	protected boolean evalintegral(GrammarRule rule, Batch ioScoreWithT, Batch ioScoreWithS, List<Double> scoreSandT) {
		...
		boolean removed = false, allocated, iallocated = false;
		...
		for (short i = 0; i < MAX_BATCH_SIZE; i++) {
			...
			ruleW.derivative(allocated, scoreT, scoreS, ggradst, ggradss, ggrads, wgrads, iosWithT, iosWithS, cachesWithT, cachesWithS);
	...
	private void normalize() {
		double grad;
		for (Entry<RuleUnit, List<List<Double>>> unit : ggrads.entrySet()) {
			for (List<Double> grads : unit.getValue()) { // every component
				for (int d = 0; d < grads.size(); d++) {
					grad = -Params.lr * Math.signum(grads.get(d)) / partition;
					grad = clip(grad);
					grads.set(d, grad);
				}
			}
		}
		for (int k = 0; k < wgrads.size(); k++) {
	...
	private void sgd() {
		double g1st, grad;
		for (Entry<RuleUnit, List<List<Double>>> unit : ggrads.entrySet()) {
			List<List<Double>> gunit0 = unit.getValue();
			List<List<Double>> gunit1 = ggrads1.get(unit.getKey());
			for (int i = 0; i < gunit0.size(); i++) {
				List<Double> grads0 = gunit0.get(i);
				List<Double> grads1 = gunit1.get(i);
				for (int d = 0; d < grads0.size(); d++) {
	...
	private void adagrad() {
		...
		for (Entry<RuleUnit, List<List<Double>>> unit : ggrads.entrySet()) {
			List<List<Double>> gunit0 = unit.getValue();
			List<List<Double>> gunit2 = ggrads2.get(unit.getKey());
			for (int i = 0; i < gunit0.size(); i++) {
				List<Double> grads0 = gunit0.get(i);
				List<Double> grads2 = gunit2.get(i);
				for (int d = 0; d < grads0.size(); d++) { // dimension d
					grad = grads0.get(d) / partition;
