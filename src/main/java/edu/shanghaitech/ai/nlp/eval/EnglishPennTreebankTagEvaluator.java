package edu.shanghaitech.ai.nlp.eval;
 ...
 * @author Dan Klein
 */
public class EnglishPennTreebankTagEvaluator<L> {
	...
	abstract static class AbstractEval<L> {
		...
		abstract Set<Object> makeObjects(List<L> tree);

		public double evaluate(List<L> guess, List<L> gold) {
		...
		public double evaluate(List<L> guess, List<L> gold, PrintWriter pw) {
		...
		private String displayPRF(String prefixStr, int correct, int guessed, int gold, int exact, int total) {
			double precision = (guessed > 0 ? correct / (double) guessed : 1.0);
			double recall = (gold > 0 ? correct / (double) gold : 1.0);
			double f1 = (precision > 0.0 && recall > 0.0 ? 2.0 / (1.0 / precision + 1.0 / recall)
					: 0.0);
			...
			String displayStr = " P: " + ((int) (precision * 10000)) / 100.0
					...
					+ ((int) (f1 * 10000)) / 100.0 + " EX: "
					+ ((int) (exactMatch * 10000)) / 100.0;
			return (prefixStr + displayStr);
		}
		
		public String display() {
			return displayPRF("[Average] ", correctEvents, guessedEvents, goldEvents, exact, total);
		}
		...
		public void reset() {
			...
			this.exact = 0;
			this.total = 0;
			this.correctEvents = 0;
			this.guessedEvents = 0;
			this.goldEvents = 0;
		}
	}
	...
	public static class UnlabeledConstituentEval<L> extends AbstractEval<L> {
		...
		@Override
		Set<Object> makeObjects(List<L> tree) {
	...
	public static class LabeledConstituentEval<L> extends AbstractEval<L> {
		...
		Set<Object> makeObjects(List<L> tree) {
			...
			for (int i = 0; i < tree.size(); i++) {
				set.add(new LabeledConstituent<L>(tree.get(i), i, i + 1));
			}
			return set;
		}
